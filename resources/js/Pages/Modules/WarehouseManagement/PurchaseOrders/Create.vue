...
const nextStep = async () => {
    if (currentStep.value === 1) {
        if (!step1Data.value.company_id || !step1Data.value.supplier_id || !step1Data.value.warehouse_id) {
        ...
        try {
            ...
            if (step2Data.value.expected_delivery_date) {
                const deliveryDate = new Date(step2Data.value.expected_delivery_date);
...
const loadSupplierProducts = async () => {
    try {
        const response = await axios.get(`/api/suppliers/${step1Data.value.supplier_id}/products`);
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                            ...
                            >
                                <div 
                                    ...
                                    :style="{
                                        ...
                                        'color': index + 1 < currentStep ? buttonPrimaryTextColor : 
                                                index + 1 === currentStep ? buttonPrimaryBgColor : 'rgb(107 114 128)'
                                    }"
                                >
                                ...
                                <div 
                                    ...
                                    :style="{
                                        'color': index + 1 === currentStep ? buttonPrimaryBgColor : 
                                                index + 1 < currentStep ? 'rgb(17 24 39)' : 'rgb(107 114 128)',
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <Autocomplete
                            ...
                            class="w-full"
                        />
                        <div v-if="step1Data.company_id" class="mt-2 p-2 bg-gray-50 rounded text-sm">
                    ...
                    <div>
                        ...
                        <Autocomplete
                            ...
                            class="w-full"
                        />
                        <div v-if="step1Data.supplier_id" class="mt-2 p-2 bg-gray-50 rounded text-sm">
                            <span class="font-medium">Selected:</span> 
                            {{ suppliers[0]?.name }}
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <Autocomplete
                            ...
                            class="w-full"
                        />
                        <div v-if="step1Data.warehouse_id" class="mt-2 p-2 bg-gray-50 rounded text-sm">
                            <span class="font-medium">Selected:</span> 
                            {{ warehouses[0]?.name }}
                        </div>
                    </div>
            ...
            <div v-if="currentStep === 2" class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6">
                <div class="grid grid-cols-1 gap-6">
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Order Date <span class="text-red-500">*</span></label>
                        <input 
                            type="date" 
                    ...
                    <div>
                        ...
                        >
                            ...
                            <option v-for="option in paymentTermsOptions" 
                                    :key="option.value" 
                                    :value="option.value">
                                {{ option.label }}
                            </option>
                    ...
                    <div>
                        ...
                        >
                            ...
                            <option v-for="option in shippingTermsOptions" 
                                    :key="option.value" 
                                    :value="option.value">
                                {{ option.label }}
                            </option>
            ...
            <div v-if="currentStep === 3" class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6">
                <div class="space-y-8">
                    <!-- Company, Supplier & Warehouse Details -->
                    <div>
                        <h3 class="text-lg font-medium text-gray-900 mb-4">Selected Organization Details</h3>
                    ...
                    <div>
                        ...
                        <div class="bg-gray-50 rounded-lg p-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <p class="mt-1">{{ formatDate("M d Y", step2Data.order_date,) }}</p>
                            </div>
                            <div>
                                <p class="text-sm font-medium text-gray-500">Expected Delivery Date</p>
                                <p class="mt-1">{{ step2Data.expected_delivery_date ? formatDate("M d Y", step2Data.expected_delivery_date) : 'Not specified' }}</p>
                            </div>
                            <div>
                                <p class="text-sm font-medium text-gray-500">Payment Terms</p>
                                <p class="mt-1">{{ paymentTermsOptions.find(opt => opt.value === step2Data.payment_terms)?.label || 'Not specified' }}</p>
                            </div>
                            <div>
                                <p class="text-sm font-medium text-gray-500">Shipping Terms</p>
                                <p class="mt-1">{{ shippingTermsOptions.find(opt => opt.value === step2Data.shipping_terms)?.label || 'Not specified' }}</p>
                            </div>
                            <div class="col-span-2">
                                <p class="text-sm font-medium text-gray-500">Notes</p>
                                <p class="mt-1">{{ step2Data.notes || 'No notes added' }}</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="p-6">
                ...
                <div class="space-y-4">
                    ...
                    <div v-if="selectedProduct" class="bg-gray-50 p-4 rounded-md">
                        <h3 class="text-sm font-medium text-gray-700 mb-2">Product Details</h3>
                        <div class="text-sm text-gray-600">
                            <p><span class="font-medium">Price:</span> {{ selectedProduct.price }}</p>
                            <p v-if="selectedProduct.description"><span class="font-medium">Description:</span> {{ selectedProduct.description }}</p>
                        </div>
                    </div>
                </div>
