...
import { singularizeAndFormat, formatNumber, getStatusPillClass } from "@/utils/global";
...
const handleReceive = async () => {
    try {
        isLoading.value = true;
        const response = await axios.post(`/api/goods-receipt-details/${selectedDetail.value.id}/receive`, receiveForm.value);
        ...
        if (response.data.results) {
            ...
            if (response.data.results.success.length > 0) {
                toast.success(`Successfully processed ${response.data.results.success.length} products`);
            }
        }
...
const handleReturn = async () => {
    try {
        isLoading.value = true;
        await axios.post(`/api/goods-receipt-details/${selectedDetail.value.id}/return`, returnForm.value);
        toast.success('Products returned successfully');
        closeReturnModal();
        router.get(`/goods-receipts/${modelData.value.id}`);
    } catch (error) {
        console.error('Error returning products:', error);
        toast.error(error.response?.data?.message || 'Failed to return products');
    } finally {
        isLoading.value = false;
    }
};
...
const addSerial = () => {
    if (receiveForm.value.has_serials) {
        const remainingQty = selectedDetail.value.expected_qty - selectedDetail.value.received_qty;
        if (receiveForm.value.serials.length >= remainingQty) {
            toast.error(`Cannot add more rows. Maximum quantity is ${remainingQty}`);
            return;
        }
    }
...
const removeSerial = (index) => {
    if (receiveForm.value.has_serials && receiveForm.value.serials.length <= 1) {
...
const submitReceive = async () => {
    try {
        ...
        if (receiveForm.value.has_serials && receiveForm.value.serials.length !== parseInt(receiveForm.value.received_qty)) {
            toast.error(`Please add exactly ${receiveForm.value.received_qty} ${receiveForm.value.type === 'serial_numbers' ? 'serial numbers' : 'batch numbers'}`);
            return;
        }

        await axios.post(`/api/goods-receipt-details/${selectedDetail.value.id}/receive`, receiveForm.value);
...
const loadDetails = async () => {
    try {
        const response = await axios.get(`/api/goods-receipt-details?goods_receipt_id=${modelData.value.id}`);
...
watch(() => modelData.value?.details, (newDetails) => {
    if (newDetails) {
...
watch(() => receiveForm.value.has_serials, (newValue) => {
    if (newValue) {
        ...
        receiveForm.value.received_qty = Math.max(receiveForm.value.serials.length, 1);
        if (receiveForm.value.serials.length === 0) {
            addSerial();
        }
    }
...
watch(() => receiveForm.value.serials.length, (newLength) => {
    if (receiveForm.value.has_serials) {
...
const isFullyReceived = computed(() => {
    if (!modelData.value?.details || modelData.value.details.length === 0) return false;
    return modelData.value.details.every(detail => detail.expected_qty === detail.received_qty);
});

// Add this method after other methods
const handleTransfer = async () => {
    if (!confirm('Are you sure you want to transfer these items to warehouse?')) return;
    
    try {
        isLoading.value = true;
        await axios.post(`/api/goods-receipts/${modelData.value.id}/transfer`);
...
<template>
    <AppLayout :title="`${singularizeAndFormat(modelName)} Details`">
        ...
        <div class="max-w-7xl mx-auto">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg pt-6">
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="px-2 py-2 bg-gray-50 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-24">Expected Qty</th>
                                        <th class="px-2 py-2 bg-gray-50 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-24">Received Qty</th>
                                        <th class="px-2 py-2 bg-gray-50 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-20" v-if="modelData.status != 'in-warehouse'">Actions</th>
                                    </tr>
                                </thead>
                                <tbody class="bg-white divide-y divide-gray-200">
                                    <tr v-for="detail in modelData.details" :key="detail.id">
                                        ...
                                        <td class="px-2 py-2">{{ formatNumber(detail.expected_qty, { minimumFractionDigits: 0 }) }}</td>
                                        <td class="px-2 py-2">{{ formatNumber(detail.received_qty, { minimumFractionDigits: 0 }) }}</td>
                                    ...
                                    <tr v-if="!modelData.details || modelData.details.length === 0">
                ...
                <div class="border-t border-gray-200 py-6">
                    <div class="px-6">
                        <h3 class="text-lg font-medium text-gray-900 mb-4">Serial/Batch Numbers</h3>
                                ...
                                <table class="min-w-full divide-y divide-gray-200">
                                    ...
                                    <tbody class="bg-white divide-y divide-gray-200">
                                        <template v-for="detail in modelData.details" :key="detail.id">
                                            <tr v-for="serial in detail.serials" :key="serial.id" class="hover:bg-gray-50">
    ...
    <!-- Receive Modal -->
    <div v-if="showReceiveModal" class="fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center p-4">
        <div class="bg-white rounded-lg p-6 w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            <h3 class="text-lg font-medium text-gray-900 mb-4">Receive Items</h3>
            ...
            <div class="space-y-4">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input 
                            ...
                            min="0"
                            :max="selectedDetail ? selectedDetail.expected_qty - selectedDetail.received_qty : 0"
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                            :disabled="receiveForm.has_serials"
                            :title="receiveForm.has_serials ? 'Quantity is based on the number of serial/batch numbers' : ''"
                        />
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700">Has Serial/Batch Numbers</label>
                        <div class="mt-1">
                            <input type="checkbox" v-model="receiveForm.has_serials" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" />
                        </div>
                    </div>
                </div>
                ...
                <div v-if="receiveForm.has_serials" class="overflow-x-auto">
                    ...
                    <div class="border rounded-lg overflow-hidden">
                        ...
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-gray-200">
                                <thead class="bg-gray-50">
                                    <tr>
                                        <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                            {{ receiveForm.type === 'serial_numbers' ? 'Serial Number' : 'Batch Number' }}
                                        </th>
                                        <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                            Manufactured Date
                                        </th>
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    <tr v-for="(serial, index) in receiveForm.serials" :key="index" class="hover:bg-gray-50">
                                        <td class="px-3 py-2">
                                            <input 
                                                type="text" 
                                                v-model="serial[receiveForm.type === 'serial_numbers' ? 'serial_number' : 'batch_number']"
                                                class="block w-full border-0 p-0 focus:ring-0 sm:text-sm"
                                                :placeholder="receiveForm.type === 'serial_numbers' ? 'Enter serial number' : 'Enter batch number'"
                    ...
                    <button 
                        @click="addSerial"
                        class="mt-4 text-sm text-indigo-600 hover:text-indigo-900 flex items-center"
                    >
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-1" viewBox="0 0 20 20" fill="currentColor">
                            <path fill-rule="evenodd" d="M10 5a1 1 0 011 1v3h3a1 1 0 110 2h-3v3a1 1 0 11-2 0v-3H6a1 1 0 110-2h3V6a1 1 0 011-1z" clip-rule="evenodd" />
                        </svg>
                        Add {{ receiveForm.type === 'serial_numbers' ? 'Serial Number' : 'Batch Number' }}
                    </button>
                </div>
            </div>
    ...
    <!-- Return Modal -->
    <div v-if="showReturnModal" class="fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center">
        <div class="bg-white rounded-lg p-6 max-w-md w-full">
